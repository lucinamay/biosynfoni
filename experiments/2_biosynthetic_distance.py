...
def compound_graph(reaction_info: pd.DataFrame) -> pd.DataFrame:
...
def id_mol_dict(struct_loc: str) -> dict:
    return {mol.GetProp("compound_id"): mol for mol in Chem.SDMolSupplier(struct_loc)}
...
def similarity(
    ...
    if not (isinstance(fp1, np.ndarray) and isinstance(fp1, np.ndarray)):
        return np.nan
    return SIM_FUNCTIONS[metric]([fp1, fp2])
...
def pairs_per_separation(chains: dict, max_separation: int = 4) -> pd.DataFrame:
    """returns a dataframe with all pairs separated by separation in chain_indexes"""
    ...
    for sep in range(1, max_separation + 1):
        ...
        pairs_records.append(records)
    pairs_records = pd.DataFrame(records).orient("index")
...
def biosynthetic_distance_analysis(pairs_df, metric):
    ...
    if not os.path.exists(dist_df) or not os.path.exists(mols_pkl):
        ...
        df = add_similarities_per_fp(df, metric=metric)
        mols = df.copy()
        # save mols as pickle
...
def main():
    ...
    compound_data = pd.read_csv(
        input_dir / "meta_compounds.tsv", sep="\t", index_col="compound_id", header=True
    )
