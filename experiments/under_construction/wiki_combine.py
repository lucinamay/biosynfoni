...
class MyTaxon:
    def __init__(
        ...
        ncbi_id,
        tax_tree="",
        taxinfo_header=("domain", "kingdom", "phylum", "subphylum", "order"),
    ):
        if not tax_tree:
            global TAX_TREE
            tax_tree = TAX_TREE
    ...
    def __ncbi_to_id__(self) -> int:
        ...
        # main functionality
        ncbi_cmd = ["grep", "ncbi:{}[^0-9+\-]".format(self.ncbi_id), self.tax_tree]
    ...
    def getparent(self, child_id: int = 0) -> int:
        ...
        par_cmd = ["grep", "^{}[^0-9+\-]".format(child_id), self.tax_tree]
    ...
    for fil in file_list:
        ...
        if fil.endswith(".json") or fil.endswith(".pickle"):
            ...
            if first == True:
                ...
                ind_df.to_csv(tsvname, sep="\t", mode="a", index=False, header=False)
            first = False
