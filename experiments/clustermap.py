...
def cli():
    ...
    parser.add_argument("-s", "--subsample", required=False, type=int, help="subsample size", default=None)
    parser.add_argument("-r". "--seed", "--randomseed", required=False, type=int, help="seed for subsampling", default=None)
    parser.add_argument("-S", "--synthetic compounds", required=False, type=str, help="format of output file", default=None)
...
class ClusterMap:
    ...
    def get_tree(self):
        return sch.dendrogram(self.clustering, leaf_font_size = 2, color_threshold=4, labels=self.indexes)
    ...
    def seacluster(self):
        ...
        with recursion_depth(20000):
            seacluster = sns.clustermap(
                ...
                cmap=cmap,
                cbar_kws={"shrink":.3, "label": "counts"},
                )
        return seacluster
    ...
    def get_clusterplot(self, legend_title="NP-Classifier prediction"):
        ...
        plt.title(f"Hierarchical clustering of compounds and substructures {self.method}, {self.metric}")# + 'z scored')
        ...
        plt.legend(legend_colors, handles.keys(), title=legend_title,
...
def main():
    ...
    npcs = pd.read_csv('../../input/coco/coconut_classes.tsv', sep='\t', header=None, dtype=str, usecols=[0])
    ...
    for method in ["average", "complete", "single", "weighted"]:
        for metric in ["euclidean", "cityblock", "cosine", "correlation", "hamming", "jaccard","mahalanobis", "chebyshev", "canberra", "braycurtis", "dice", "kulsinski", "matching", "rogerstanimoto", "russellrao", "sokalmichener", "sokalsneath", "yule"]:
