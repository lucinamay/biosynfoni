...
def get_pw_tree(reaction_layouts:list[str], max_lvl:int=5)->list[list[str]]:
    ...
    reaction_tree = make_tree_dict(parent,
                                   prec_prods,
                                   parent_lvl=0,
                                   max_lvl=max_lvl)
...
def write_reactions(pathways, dict1, max_level=7,
                    dict2={},dict3={},
                    title='metacyc_reactions', annot:dict={}, fulldf=None):
    ...
    assert(len(all_pw_ids) == len(all_reaction_layouts)),\
    ...
    for i in range(len(all_reaction_layouts)):
        ...
        if annot:
            ...
            if fulldf is None:
                continue
            taxonomic_range = fulldf[fulldf['UNIQUE-ID']==all_pw_ids[i][0]][
                'TAXONOMIC-RANGE'].unique().tolist()
...
def main():
    ...
    info2extract = ['UNIQUE-ID','REACTION-LIST','SPECIES','TAXONOMIC-RANGE','REACTION-LAYOUT']
    ...
    pathways_oi = get_relevant_pws(pathways,
    ...
    write_reactions(pathways_oi,main_dict,
                    max_level=7,
