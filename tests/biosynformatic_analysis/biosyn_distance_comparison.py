...
def _fps_to_distance(fp1, fp2, metric='c_tanimoto'):
    ...
    assert isinstance(fp1, np.ndarray) and isinstance(fp2, np.ndarray),\
...
def get_pairs_dist_df(
    ...
    df = pd.DataFrame({
        "pathway": pathways,
        "precursor": prec_strings,
        "product": prod_strings
    })
    ...
    for fp_type in FP_FUNCTIONS.keys():
        df[fp_type] = [similarity(
            x[0], x[1],
            fingerprint=fp_type,
            metric=metric,
            check_moliness=False) for x in molpairs]
    return df
...
def _get_comparison_df(struct_loc, max_steps=4, metric='c_tanimoto'):
    ...
    for i in range(1, (max_steps+1), 1):
        ...
        degree_of_sep = i
        dictionary = dictify_pw_pairs(
            struct_loc, degree_of_sep=degree_of_sep)
...
def get_square(
    ...
    square = df[
        (df[col1] > range1[0]) & (df[col1] < range1[1]) & (
            df[col2] > range2[0]) & (df[col2] < range2[1])
    ]
    return square


def draw_molpair(pair: list[Chem.Mol], outfilename: str, highlighting: bool = True) -> None:
...
def draw_squares(
    ...
    for i in square_df.index:
        pair = [str_to_mol(square_df[pair_columns[0]][i]),
...
def main():
    ...
    annotated_df = annotate_pathways(
        comparison_df, pw_tax_file, tax_text_file)
    ...
    left_bottom = get_square(one_step, 'biosynfoni',
                             'rdkit', (0, 0.2), (0, 0.2))
    left_top = get_square(one_step, 'biosynfoni',
                          'rdkit', (0, 0.2), (0.8, 1))
    right_bottom = get_square(one_step, 'biosynfoni',
                              'rdkit', (0.8, 1), (0, 0.2))
    right_top = get_square(one_step, 'biosynfoni',
                           'rdkit', (0.8, 1), (0.8, 1))
